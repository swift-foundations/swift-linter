...
extension Lint.File.Single {
    ...
    public enum Classification: Swift.Sendable, Swift.Equatable {
        /// The consumer activates exactly ONE baked standard bundle
        /// (`bundle.expression` — e.g. `Lint.Rule.Bundle.primitives` or
        /// `Lint.Rule.Bundle.standards`) with no per-consumer selection, no
        /// inline/custom rules, no parent chain, and only standard rule-pack
        /// dependencies. Its active rule set is byte-for-byte the set the
        /// standard runner bakes for that token, so the runner reproduces
        /// the eval result without recompiling. The token rides the spawn's
        /// ``Lint/Rule/Bundle/Baked/Channel`` so the runner selects the
        /// consumer's bundle — never a substituted one.
        case fastPathStandardBundle(bundle: Lint.Rule.Bundle.Baked)

        /// The consumer activates exactly
        /// `<bundle.expression>.excluding(rules: [...])` — a baked standard
        /// bundle minus a set of per-package exclusions — with no
        ...
        /// the result by linting the baked `bundle` registry with
        ...
        /// non-baked bundle, per-consumer enable/disable/exclude,
